...
static void FigureListDrawFunc(UInt16 itemNum, RectanglePtr bounds, Char *data[]);
...
static Boolean MainFormButtonHandler(FormPtr frm,EventPtr event) {
  ...
  case buttonEdit: {
    ErrFatalDisplayIf(!CheckROMVerGreaterThan(3,5),"You need Palm OS >= 3.5");
...
static void FigureListFill(FormPtr frm) {
  ...
  ListPtr list = (ListPtr)FrmGetObjectPtr(frm,FrmGetObjectIndex(frm, listFigure));
...
static void FigureListDrawFunc(UInt16 itemNum, RectanglePtr bounds, Char *data[]) {
  ...
  WinDrawChars(currFigure,StrLen(currFigure), bounds->topLeft.x+6, bounds->topLeft.y);
}
