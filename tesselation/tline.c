...
TVertexnode_type *TLineBreakAtXY(TLinenode_type *l1,double x,double y) {
  ...
  while (otv->next!=NULL) {
    ...
    if (_abs(TPointDistanceFromLine(pm,otv->p1,otv->next->p1))<POINTSIZE) {
    ...
    if (_abs(TPointDistanceFromLine(pm,otv->p2,otv->next->p2))<POINTSIZE) {
...
void TLineTesselate(TLinenode_type l1,int x,int y,double scale,double ca,double sa) {  
  TVertexnode_type *otv=l1.rootnode;
  int ox,oy;
  int nx,ny;
  ox=(otv->p1.x*ca-otv->p1.y*sa)*scale+x;
  oy=(otv->p1.x*sa+otv->p1.y*ca)*scale+y;
  while (otv->next!=NULL) {
    nx=(otv->next->p1.x*ca-otv->next->p1.y*sa)*scale+x;
    ny=(otv->next->p1.x*sa+otv->next->p1.y*ca)*scale+y;
...
void TLinenodeAdd(TLinenode_type *rootnode,
	     double x1,double y1,
	     double x2,double y2,
	     double x3,double y3,
	     double x4,double y4,double rot) {
